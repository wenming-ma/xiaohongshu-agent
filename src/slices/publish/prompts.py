...
- 检查页面是否有登录提示或登录按钮
- **如果未登录**：
  * 向用户报告：**检测到未登录状态，请在浏览器中手动登录小红书创作平台**
  * 等待 60 秒，让用户有时间登录
  * 每隔 10 秒检查一次登录状态（共检查 6 次）
  * 如果检测到已登录，报告"✅ 登录成功，继续执行发布流程"
  * 如果 60 秒后仍未登录，报错：**超时错误：用户未在 60 秒内完成登录**
- **如果已登录**：
  * 报告"✅ 已登录，继续执行发布流程"
  * 继续下一步
...
### 步骤 6：填写正文
- 定位正文编辑器（可能是富文本编辑器，如 contenteditable DIV）
- 清空现有内容（如果有）
- **完整输入**提供的正文内容（正文已包含行动号召和话题标签，不要遗漏任何部分）
- **重要**：正文末尾已包含 #话题 格式的标签，直接粘贴即可，不需要使用 "# 话题" 按钮单独添加
- 注意排版，比如正确换行，空格等

### 步骤 7：点击发布（跳过单独添加话题步骤）
...
2. **正文和话题**：
   - 正文末尾已包含 #话题 格式的标签（如 #西安求职 #避坑指南）
   - **不要使用 "# 话题" 按钮单独添加话题**，直接输入完整正文即可
...
- **正文末尾已包含 #话题 标签**，直接输入正文即可，不要使用 "# 话题" 按钮单独添加

## 执行要求

1. 严格按照系统提示的 9 个步骤执行（已跳过单独添加话题步骤）
2. 每完成一个关键步骤后，简要报告进度
3. 遇到问题立即停止并报告详细错误
4. 发布成功后，尝试获取发布链接
