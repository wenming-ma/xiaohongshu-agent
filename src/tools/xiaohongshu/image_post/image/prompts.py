...
### ⚠️ 实体名称模糊处理（通用规则，所有图片必须遵守！）
图片中涉及的**具体公司名、品牌名、店铺名、机构名**等实体名称，必须进行模糊处理：
- 用 **X** 或 **\*** 替换名称中的部分文字
- 示例：华为 → 华X / 华*、星巴克 → 星*克、海底捞 → 海*捞
- 原则：保留足够特征让读者能猜到，但不直接点名（规避法律风险）
- 适用范围：无论推荐、避雷、测评、攻略还是其他任何类型，均需模糊处理
...
4. **实体名称模糊处理（通用规则）**：所有帖子中的公司名/品牌名/店铺名/机构名均需用X或*替换部分文字（如：华X、星*克）
