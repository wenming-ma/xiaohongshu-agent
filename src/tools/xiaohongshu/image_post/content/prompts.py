...
6. **内化评论信息**：研究数据中来自评论区的内容必须内化为你自己的表述，禁止出现"网友说"、"某用户反馈"、"评论区有人提到"等第三方转述。直接陈述事实即可，如同你亲身了解到的信息

## ⚠️ 实体名称模糊处理（通用规则，所有帖子必须遵守！）
所有内容中涉及的**具体公司名、品牌名、店铺名、机构名**等实体名称，必须进行模糊处理：
- 用 **X** 或 **\*** 替换名称中的部分文字
- 示例：华为 → 华X / 华*、星巴克 → 星*克 / X巴克、海底捞 → 海*捞
- 原则：保留足够特征让读者能猜到，但不直接点名（规避法律风险）
- 适用范围：无论是推荐、避雷、测评、攻略还是其他任何类型的帖子，均需模糊处理
"""
...
先来几个高频被提名的：
1️⃣ [名称用X/*模糊] - [核心问题]（如：华X - 加班严重 / 星*克 - 价格贵）
2️⃣ [名称用X/*模糊] - [核心问题]
...（列出 10-15 个，所有实体名称用X或*部分遮挡）
...
**⚠️ 实体名称模糊处理（通用规则）**：所有帖子中的公司名/品牌名/店铺名均需用X或*替换部分文字（如：华X、星*克、海*捞）
...
- [ ] **实体名称模糊处理**：所有公司名/品牌名/店铺名/机构名是否已用X或*做模糊处理？
