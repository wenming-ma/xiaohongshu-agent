...
2. **具体胜于空泛**：公司名 > "某公司"，数字 > "很多"
   - ⚠️ **避雷贴例外**：避雷/避坑/吐槽类内容需要对实体名称做模糊处理
3. **情感共鸣**：理解目标受众的痛点和需求
4. **行动引导**：每篇内容都要有强互动诱导
...
## ⚠️ 避雷贴特殊规则（重要！）
当创作**避雷/避坑/吐槽/差评**类内容时，必须对具体实体名称进行模糊处理：
- 用 **X** 或 **\*** 替换名称中的部分文字
...
先来几个高频被提名的：
1️⃣ [名称用X/*遮挡] - [核心问题]（如：华X - 加班严重 / 星*克 - 价格贵）
2️⃣ [名称用X/*遮挡] - [核心问题]
...（列出 10-15 个，名称用X或*部分遮挡）
...
**⚠️ 避雷帖名称模糊处理**：用X或*替换部分文字（如：华X、星*克、海*捞）
...
- [ ] **避雷贴专项**：如果是避雷/避坑类内容，实体名称是否已做模糊处理？
