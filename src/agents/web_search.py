"""
WebSearch 工具（给 Agent 调用）

说明：
- 这是“repo 内部实现”的 web search（不依赖模型 provider 的 builtin web_search）。
- 目的：让 Research/Search 流程可以快速在全网检索补充关键词/背景信息。
- 默认实现走“无需 key 的兜底”（DuckDuckGo Instant Answer API），
  如果你配置了 API Key，会自动使用更完整的搜索后端：
  - SERPER_API_KEY: https://serper.dev/（Google Serper）
  - TAVILY_API_KEY: https://tavily.com/（Tavily）
"""
...
class WebSearchAgent:
    ...
    async def web_search(self, query: str, max_results: int = 5, deep_search: bool = False) -> str:
        ...
        Args:
            query: 搜索关键词
            max_results: 最大返回条数（不同后端会尽量对齐）
            deep_search: 是否启用 deep search（扩写 query、多轮搜索、合并去重）
        ...
        Returns:
            JSON 字符串：
            {
              "query": "...",
              "deep_search": true,
              "backend": "serper|tavily|duckduckgo_*|mixed",
              "results": [{"title": "...", "url": "...", "snippet": "..."}],
              "answer": "...",
              "claims": [{"claim":"...","detail":"...","sources":["..."],"confidence":"high"}],
              "coverage_notes": "...",
              "error": ""
            }
        """
    ...
    async def _search_duckduckgo_instant_answer(self, query: str, max_results: int) -> dict[str, Any]:
        ...
        注意：这不是“完整 SERP”，但足够用于：
        - 快速拿到定义/摘要
        - 获取一些 related topics（用于扩展关键词）
        """
        params = {
            "q": query,
            "format": "json",
    ...
    async def _search_duckduckgo_html(self, query: str, max_results: int) -> dict[str, Any]:
        ...
        作为 Instant Answer 为空时的兜底，用于获取“真实搜索结果链接+摘要”。
        """
        params = {"q": query}
        headers = {
            # 尽量像普通浏览器，避免被当成机器人
        ...
        # DuckDuckGo HTML 结构：result__a（标题）后面跟着 result__snippet（摘要），通常同一条结果内顺序稳定
        pat = re.compile(
    ...
    async def _synthesize(self, query: str, results: list[dict[str, str]]) -> WebSearchSynthesisResult:
        """把搜索结果整合为调用者想要的答案（结构化 claims + sources）。"""
    ...
    def _canonicalize_url(url: str) -> str:
        """用于合并/去重的 URL 规范化：去掉 fragment，移除常见追踪参数。"""
        if not url:
            return ""
        try:
            parts = urlsplit(url)
    ...
    def _merge_rank_results(
        ...
        """合并多个 query 的搜索结果，按覆盖面+排名进行排序并去重。"""
    ...
    async def _search_many(self, queries: list[str], per_query_k: int) -> dict[str, dict[str, Any]]:
        """并发执行多个搜索，返回 per-query 结构（带 backend/error）。"""
    ...
    async def _deep_search(self, query: str, max_results: int) -> tuple[str, list[dict[str, str]], dict[str, Any]]:
        ...
        # Round 2 expand from top snippets
        try:
