...
### 步骤 4.5: 上传自定义封面（如果提供了封面图片）
- 如果任务中包含封面图片路径，则：
  1. 视频处理完成后，找到封面/缩略图区域，点击「选择封面」或「更换封面」
  2. 在弹出的封面选择界面中，找到「上传封面」按钮并点击
  3. 使用 setInputFiles() 上传封面图片文件
  4. 等待封面预览显示，确认上传成功
  5. 点击「确定」或「完成」保存封面选择
- 如果未提供封面图片，跳过此步骤，使用平台默认缩略图
...
1. 严格按照系统提示的步骤执行
2. 视频上传后等待处理完成
3. 如果封面图片路径不为空，按步骤 4.5 上传自定义封面
4. **话题通过推荐芯片添加**：正文输入后点击下方推荐话题芯片即可，不要使用「# 话题」按钮
5. 话题必须作为平台识别的独立话题存在，不能挂在正文末尾成为普通文本
6. 发布成功后尝试获取发布链接
