...
### 文字规范（默认少字/无字，避免 AI 错字）
- **默认原则**：普通写实、产品、穿搭、生活方式图片优先做纯视觉图，不生成标题、副标题、海报字卡或大段文字；标题和正文交给飞书交付包承载。
- **只有当前任务明确要求文字海报、信息图、知识卡或图内文字时**，才允许少量文字。
- **封面图**：如果确实需要文字，主标题 8-15 个汉字，可选副标题 10 个字以内；否则用纯视觉主图突出主题。
- **详情图**：信息图风格可用 4-6 个要点；其他风格应完全无文字或只有极短标签。
- **所有文字必须是简体中文，且必须准确可读**
