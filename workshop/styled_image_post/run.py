"""
迭代执行 workshop/styled_image_post/topics.json 中的话题，调用 StyledImagePostPipeline。

用法:
    uv run python workshop/styled_image_post/run.py
    uv run python workshop/styled_image_post/run.py --start-index 3
    uv run python workshop/styled_image_post/run.py --start-index 5 --limit 2
    uv run python workshop/styled_image_post/run.py --sleep 3600
"""
...
async def run_batch(args: argparse.Namespace) -> int:
    ...
    summary_path = SCRIPT_DIR / f"styled_image_post_summary_{ts}.json"
    summary = {
        "timestamp": ts,
        "total": total,
    ...
    if failed:
        failed_path = SCRIPT_DIR / f"styled_image_post_failed_{ts}.json"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="批量执行 XHS Styled Image Post（支持参考图片指定推荐物品样式）")
